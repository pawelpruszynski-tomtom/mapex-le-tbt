...
# MAGIC %md
# MAGIC ## Training data
# MAGIC Define the path of the training master table and the features used

# COMMAND ----------

training_master_table = "hive_metastore.error_classification.training_table"
...
# MAGIC %md
# MAGIC ## Preprocessing
# MAGIC Define if you want to rescale all feature columns to be centered around zero with unit variance.
...
storage_account_access_key = 'iejB8uYqdZ06kxuKHvDXWbUHuYc41R/K5sFxmMDJgLCup8kQrNUH+U33ZcbyqpvRp9jnhyDDZcVa2NMjxW91ZA=='
...
def iterate_threshold(y_true, y_probs, steps=100, saved_error_cost=2, missed_error_cost=10):
    """
...
# MAGIC %md
# MAGIC ## Feature Importance function

# COMMAND ----------

model_dict["pipeline"][-1].base_estimator
...
def log_feature_importance(model, feature_cols):
    ...
    if type(classifier) == sklearn.calibration.CalibratedClassifierCV:
        classifier = classifier.base_estimator

    fimpo_df = pd.DataFrame([feature_cols, classifier.feature_importances_]).T
    fimpo_df.columns = ['column_name', 'rf_importance']
    fimpo_df.sort_values(by='rf_importance', ascending = False)

    mlflow.log_table(fimpo_df, 'fimpo/rf_fimpo.json')
...
def log_shap(model, x_train, x_val, n_min = 200):
    ...
    shap.summary_plot(shap_values, train_sample, plot_type="bar", show=False)
    plt.savefig('shap_barplot.png')
    mlflow.log_artifact('shap_barplot.png', 'fimpo/')
    plt.close()

    # Generate SHAP beeswarm plot
    shap.summary_plot(shap_values, example, class_names=model.classes_, show=False)
    plt.savefig('shap_beeswarm_plot.png')
    mlflow.log_artifact('shap_beeswarm_plot.png', 'fimpo/')
    plt.close()
    ...
    mlflow.log_table(importance_df, 'fimpo/shap_fimpo.json')
...
def run_trial(
    ...
    with mlflow.start_run() as run:
        ...
        mlflow.log_artifact('val_conf_mat.png', 'conf_mat/')
        ...
        mlflow.log_artifact('test_conf_mat.png', 'conf_mat/')
        

        ## Log CSV to MLflow
        ## Write csv from stats dataframe
        threshold_iterations_val.to_csv('threshold_iterations_val.csv')
        threshold_iterations_test.to_csv('threshold_iterations_test.csv')
        ## and then log
        mlflow.log_artifact('threshold_iterations_val.csv')
        mlflow.log_artifact('threshold_iterations_test.csv')
        ...
        # log feature importance and shap feature importance
