...
# TODO: Add Flags here
artificial_inspection_data_option = "option_3"
...
# load model 2 (typically runner-up model)
"""
TOP models: 
uncalibrated current runner-up: 3395a062ac6e4c08a3d240287c1808ff
calibrated current runner-up: 326aae69b6064f92babce7b68655ac25

OTHER models:
uncalibrated runner-up trained only on FCD available: fb8a9ac75d0c4a6cb7accd06d650baec
uncalibrated runner-up trained on TbT only: 413fb4f00aaf48d28bcec0958386e304
uncalibrated runner-up trained on HDR only: 189572e85c89495b8774f1882764f0d0

calibrated runner-up trained on TbT only: 712c7517d2be4f2e86147ebbe9dde9a3
calibrated runner-up trained on HDR only: 69d14861e2be4482b1e059e9f15955eb
"""

                                                                                            # Conventions: 
loaded_model_2 = mlflow.sklearn.load_model(f'runs:/3395a062ac6e4c08a3d240287c1808ff/model') # new model
loaded_model_3 = mlflow.sklearn.load_model(f'runs:/413fb4f00aaf48d28bcec0958386e304/model') # new model for TbT only
loaded_model_4 = mlflow.sklearn.load_model(f'runs:/189572e85c89495b8774f1882764f0d0/model') # new model for HDR only 
...
# MAGIC %md
# MAGIC # Load data
# MAGIC The idea is to compare the models on different datasets: 
# MAGIC
# MAGIC - **test_data**: 5000 cases that were retained during model training for testing purposes
# MAGIC - **artificial_inspection**: 400 cases that were initially discarded by the old model, and sent to MCP for validation
# MAGIC - **discards_pdf**: all cases that were discarded by the old model
# MAGIC - **combined_data**: mix of discard_pdf and test_data for which MCP ground truth labels are available
# MAGIC   - Test data: 5000 rows
# MAGIC   - discard data: 5000 rows (sample)
# MAGIC - **balanced_data**: mix of: 
# MAGIC   - test_data (sample, n=4992): **not discarded** by the initial model --> MCP ground truth labels are available.
# MAGIC   - artificial_inspection (n=400): **discarded** by the intial model --> MCP labels are still available.
# MAGIC   - To match the average discard rate since the model was put into production (13.34%) only xx percent of the the test_data are sampled
# MAGIC
# MAGIC
# MAGIC All datasets need to have at least the following columns to conduct the comparisons: 
# MAGIC - has_error: ground truth label, 0 or 1
# MAGIC - prediction_1
# MAGIC - prediction_2
# MAGIC - probability_1
# MAGIC - probability_2
...
# load test data
test_table = spark.read.format('delta').load(test_table_with_old_model_predictions_path)
...
test_table = (
    test_table
    # add run_id
    .join(ics.select("case_id", "run_id"),
          on = ["case_id"],
          how = "left") 
    # add product
    .join(im.select("run_id", "product"),
          on = ["run_id"],
          how = "left") 
)
