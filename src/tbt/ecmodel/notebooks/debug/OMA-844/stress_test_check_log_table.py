...
# MAGIC %md
# MAGIC ### Analysis: compare new EC process of using submodule & new models to old process
# MAGIC
# MAGIC See ticket https://jira.tomtomgroup.com/browse/OMA-884
# MAGIC
# MAGIC Run a couple of inspections with the old version and with the new versions and compare:
# MAGIC - Timing/Latency
# MAGIC - potential errors
# MAGIC   - Number of potential errors
# MAGIC   - errors discarded/not discarded by both/only old/only new

# COMMAND ----------

# DBTITLE 1,Connect to dev database
# connect to dev or prod database
ENVIRONMENT = "dev"
...
# DBTITLE 1,Get inspection_metadata
query = """
select 
im.run_id::varchar,
im.sample_id::varchar,
im.inspection_date,
im.mcp_tasks
from inspection_metadata im
where im.product = 'OMA-884-submodule'
"""

pdf = pd.read_sql_query(query, tbt_engine)
pdf

# COMMAND ----------

# DBTITLE 1,Get the critical sections
query = """
select 
run_id, route_id, case_id, fcd_state, stretch_length
from inspection_critical_sections ics
where ics.run_id 
in (
	select im.run_id from inspection_metadata im 
	where im.product = 'OMA-884-submodule'
)
"""

critical_section = pd.read_sql_query(query, tbt_engine)
critical_section.head()

# COMMAND ----------

# DBTITLE 1,Calculate errors discarded/not discarded by both/only old/only new
# find critical section pairs based on route_id and stretch_length 
critical_section
...
# MAGIC %md
# MAGIC ## Check API log table
# MAGIC

# COMMAND ----------

# DBTITLE 1,Show API log table to compare columns
api_log_table_path = 'abfss://directions-metric@mcpengbronze.dfs.core.windows.net/ec_model_api/prod/logs.delta'
spark.read.format("delta").load(api_log_table_path).display()

# COMMAND ----------

# DBTITLE 1,Check existing files in datalake
dbutils.fs.ls('abfss://directions-metric@mcpengbronze.dfs.core.windows.net/ec_model_api/prod')

# COMMAND ----------

dbutils.fs.ls('abfss://directions-metric@mcpengbronze.dfs.core.windows.net/tbt/dev/ec_model_submodule/')
